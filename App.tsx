  ...
  return (
    <div className="min-h-screen relative bg-[#F5F5F5] selection:bg-[#D75437] selection:text-white overflow-x-hidden">
      ...
      <nav className="fixed top-0 left-0 w-full px-4 md:px-20 py-4 md:py-12 flex justify-between items-center z-[500] pointer-events-none">
        ...
        >
          {/* Logo 容器：通过滤镜增强融入感 */}
          <div className="relative w-12 h-12 md:w-20 md:h-20 overflow-hidden flex items-center justify-center">
             <img 
                src={ASSETS.logo} 
                className="w-full h-full object-contain filter brightness-110 contrast-110 drop-shadow-[0_0_12px_rgba(255,255,255,0.5)]" 
                alt="Unio Logo" 
             />
          </div>
        ...
        <div className="pointer-events-auto flex items-center gap-4 md:gap-12 text-black/40">
           <button onClick={() => navigateToView('brand-studio')} className="p-3 rounded-full hover:bg-black/5 transition-colors hover:text-black/60 active:scale-90 shadow-sm"><Settings size={20} /></button>
      ...
      <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-[600] pointer-events-none w-full max-w-lg px-4">
        <div className="pointer-events-auto flex items-center justify-around glass px-10 py-5 rounded-full border border-black/5 shadow-[0_25px_60px_rgba(0,0,0,0.15)]">
