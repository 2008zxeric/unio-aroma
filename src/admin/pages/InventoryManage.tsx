...
export default function AdminInventory() {
  ...
  // 组字状态（useState 触发重渲染 → useMemo 正确响应）
  const [purComposing, setPurComposing] = useState(false);
  const [salComposing, setSalComposing] = useState(false);

  // 拼音模糊搜索缓存（避免 JSX 中重复 filter）
  const filteredPurProducts = useMemo(
    () => purSearchText && !purComposing ? products.filter(p => p.is_active !== false && matchProduct(p, purSearchText)) : [],
    [products, purSearchText]
  );
  const filteredSalProducts = useMemo(
    () => salSearchText && !salComposing ? products.filter(p => p.is_active !== false && matchProduct(p, salSearchText)) : [],
                ...
                <div>
                  ...
                  <div className="relative">
                    {purchaseForm.product_id ? (
                      ...
                      <>
                        <Search size={13} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-[#9AAA9A]" />
                        <input type="text" value={purSearchText} onChange={(e) => setPurSearchText(e.target.value)} onCompositionStart={() => setPurComposing(true)} onCompositionEnd={(e) => { setPurComposing(false); setPurSearchText(e.currentTarget.value); }} placeholder="输入产品名称搜索..." className={`${inputCls} pl-8`} />
                        {purSearchText && (
                ...
                <div><label className="block text-xs text-[#6B856B] mb-1.5">选择产品 *</label>
                  <div className="relative">
                    {saleForm.product_id ? (
                      ...
                      <>
                        <Search size={13} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-[#9AAA9A]" />
                        <input type="text" value={salSearchText} onChange={(e) => setSalSearchText(e.target.value)} onCompositionStart={() => setSalComposing(true)} onCompositionEnd={(e) => { setSalComposing(false); setSalSearchText(e.currentTarget.value); }} placeholder="输入产品名称搜索..." className={`${inputCls} pl-8`} />
                        {salSearchText && (
                          <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-[#E0ECE0] rounded-lg shadow-lg z-50 max-h-48 overflow-y-auto">
