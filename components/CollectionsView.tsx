...
const ProductCard: React.FC<ProductCardProps> = ({ 
  ...
  >
    <div 
      className="relative aspect-[3/4] rounded-[2rem] sm:rounded-[4rem] overflow-hidden bg-white border border-black/[0.03] shadow-sm group-hover:shadow-2xl transition-all duration-1000 cursor-pointer"
...
const CollectionsView: React.FC<{ 
  ...
  const themes = {
    yuan: { label: '元 · 单方', en: 'ORIGIN · SINGLES' },
    he: { label: '香 · 复方', en: 'SCENT · HARMONY' },
    jing: { label: '境 · 空间', en: 'SANCTUARY · AROMA' }
  };
            ...
            <div className="flex bg-stone-100 p-1.5 rounded-full border border-black/[0.05] shadow-inner shrink-0">
              {(['yuan', 'he', 'jing'] as Category[]).map(c => (
                <button 
                  key={c} 
                  onClick={() => setFilter(c)} 
                  className={`text-[9px] sm:text-sm tracking-[0.2em] sm:tracking-[0.4em] uppercase font-bold px-8 py-3 sm:px-16 sm:py-5 rounded-full transition-all duration-500 whitespace-nowrap ${filter === c ? 'bg-white text-black shadow-lg scale-105' : 'text-black/30 hover:text-black/60'}`}
                >
                  {themes[c].label}
                </button>
              ))}
          ...
          {(Object.entries(groups) as [string, ScentItem[]][]).map(([groupName, groupItems]) => (
            <section key={groupName} className="space-y-10 sm:space-y-24">
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-12 lg:gap-20">
                <div className="col-span-1 bg-[#FAF9F6] rounded-[2rem] sm:rounded-[4.5rem] p-6 sm:p-14 flex flex-col justify-between border border-black/[0.03] relative overflow-hidden group shadow-sm aspect-[3/4]">
