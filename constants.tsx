...
const ERIC_JOURNAL: Record<string, string> = {
  ...
  // 全球目的地
  'w_thai': '清迈的街头，橘色的僧袍掠过碧绿的蕉叶。虔诚是不动声色的，像极了空气中常年萦绕的那抹微辛。',
  'w_is': '雷克雅未克的风能吹走所有的杂念。在黑沙滩的尽头，我意识到，人类的渺小在自然的荒凉面前，竟是一种至高的解脱。',
  'w_fr': '格拉斯的午后，阳光是金色的液体。这里不是香水的工厂，而是人类试图用玻璃瓶锁住自然的诗性野心。',
  'w_jp': '京都的苔藓里藏着一种寂静。那是时间在潮湿的石阶上缓慢爬行留下的痕迹，非入禅不可见。',
  'w_eg': '尼罗河边的微风掠过古老的法老石柱。在这里，永恒不是一个抽象词，而是一种被烈日晒出的没药味。',
  'w_sa': '开普敦的好望角，两大洋的交汇。那种澎湃的生命力，让所有的文明都显得像是暂时的借居。',
  
  // 神州目的地
  'cn_西藏': '冈仁波齐脚下，经幡在咸湿的风中飞舞。在这里，呼吸是重的，但灵魂是轻的。',
  'cn_浙江': '龙井村的雨是墨色的。在茶烟袅袅中，我终于读懂了江南对“淡泊”二字最极致的诠释。',
  'cn_云南': '横断山脉的云永远低垂。在那里，生姜不仅是本草，更是山民在湿冷雨夜里相依为命的火种。',
  'cn_新疆': '赛里木湖的蓝是不讲理的。空气里有冰川融水的冷冽，那是大自然从未被人类惊扰过的初吻。',
  'cn_北京': '红墙金瓦下的秋风，带着一种肃穆。胡同深处，你可以听到百年前的鸽哨与现代节奏的对冲。',
  'cn_四川': '青城山的翠色能渗透进皮肤。在那里，水气与泥土的混合味，就是“道法自然”最直观的注脚。'
};
...
const ALICE_LAB_DIARY: Record<string, string> = {
  ...
  '野性香茅': '高柠檬醛含量（38%），天然驱蚊效力达DEET的82%。在微乳化工艺下，敏肌刺激性降至0.03%。',
  '冰川尤加利': '1,8-桉叶素占比76%，呼吸道清透力极强。其分子具有独特的“负压感”，能即时缓解胸腔紧绷。',
  '原野茶树': '萜品烯-4-醇≥42%，抗菌谱极广。在实验室环境中，其对表皮葡萄球菌的抑制率高达99.8%。',
  '巅峰薄荷': '左旋薄荷酮纯度91%。GC/MS 监测显示其对神经末梢的即时冷觉感受器激活率提升40%，为晨间唤醒之首。',
  '老山檀香': '心材醇沉≥30年，α-檀香醇含量达55%。分子结构极其稳定，能产生持续2小时以上的Theta脑波共振。',
  '神圣花梨木': '氧化后木质甜香更显。搭配玫瑰可提升边缘系统的愉悦感反馈（fMRI验证提升37%）。',
  '烟雨丝柏': '高倍浓缩单宁，收敛微血管效果显著。其木质音频段分子的镇静率优于普通精油2.4倍。',
  '喜马雪松': '倍半萜含量极其丰富。作为基调可显著延长周围分子的挥发时长，助眠配方中入睡速度提升22%。',
  '北地松针': '富含α-蒎烯，森林浴效应明显。办公场景扩香可将血氧饱和感模拟提升，专注力均值增长15%。',
  ...
  '琥珀安息香': '天然苯甲酸酯带来丝滑的香草甜韵。对舒缓神经性干咳有显著的生理闭环反馈，感官安抚力极强。'
};

const ERIC_PROVINCE_VIBE: Record<string, string> = {
  '西南': '山川的重叠让时间慢了下来，在这里，每一缕香气都带着原始森林的湿度和远古的梦境。',
  '西北': '大漠孤烟。风沙磨砺出的骨骼感，是繁华都市永远无法企及的生命力。',
  '华南': '季风与海洋的交汇。空气是热烈而流动的，仿佛每一口呼吸都在与大海交换秘密。',
  '华东': '在精致与古老之间，香气是最好的过渡，像是一场从未醒来的园林旧梦。',
  '华北': '厚重的历史与现代的秩序。在凛冽的冬日里，需要一抹温热的木质调来守护内心的安定。',
  '东北': '黑土地的广袤，以及风穿过红松林的呼啸，那是大地最原始的野性呼吸。'
};
...
const PRODUCT_SPECIFIC_NARRATIVES: Record<string, string> = {
  '神圣乳香': '琥珀般的泪滴，承载着数千年的古老安宁。',
  '大马士革玫瑰': '晨曦中采摘的灵魂，是三千公斤花瓣凝结成的永恒。',
  '老山檀香': '时间的醇厚在指尖绽放，让躁动的思绪重归原位。',
  '巅峰薄荷': '那是阿尔卑斯山巅的风，在眉心处炸裂开的清醒。',
  '止语雾': '岩兰草的沉降感，与雪松的广袤达成了微妙的频率对冲。'
};
...
const addP = (cat: 'yuan'|'he'|'jing', group: string, n: string, en: string, folder: string, id: string, price: string, spec: string, filenameOverride?: string) => {
  ...
  DATABASE[id] = {
    ...
    narrative: PRODUCT_SPECIFIC_NARRATIVES[n] || CATEGORY_NARRATIVES[cat](n),
    ...
    aliceDiary: `我们在实验室尝试将 ${n} 的分子结构完整封存，这是一种跨越经纬的原始频率连接。`,
    aliceLabDiary: ALICE_LAB_DIARY[n] || `GC/MS 质谱分析揭示了 ${n} 非凡的化学结构与频率序位。`, 
...
const addD = (id:string, n:string, en:string, reg:string, c:number, img:string, pIds: string[] = [], s:'arrived'|'locked'='arrived', isCN:boolean=false, sub?:string, mPhots?: string[]) => {
  ...
  if (!diary && isCN && sub) {
    diary = ERIC_PROVINCE_VIBE[sub] || `踏上 ${n}，在极地边缘，我找到了共鸣的本草分子档案。`;
  }
  if (!diary) {
    diary = `在 ${n}，我意识到大地的呼吸比任何人类的言语都更具治愈力量。`;
  }
  ...
  DESTINATIONS[id] = {
    ...
    ericDiary: diary, 
    aliceDiary: `我们在实验室对 ${n} 的生态样本进行了分段冷压提取，确保其极境记忆被完整保留。`, 
