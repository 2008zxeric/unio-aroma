...
// --- [元系列：严格按照品牌方最新价格更新] ---
const yuanData = [
  { group: 'Metal', folder: 'metal', items: [
  ...
  { group: 'Water', folder: 'water', items: [
  ...
  { group: 'Fire', folder: 'fire', items: [
    ...
    ['极境天竺葵', 'Geranium Rosé', '98', '10ml']
  ]},
  { group: 'Earth', folder: 'earth', items: [
...
heData.forEach((g) => g.items.forEach((item, j) => {
  addP('he', `香 · ${g.group}`, item[0], item[1], g.folder, `he_${g.folder}_${j}`, item[2], item[3], item[4]);
}));

// --- [境系列：准确对位表格价格与文件名] ---
const jingDataA = [
  ['裂纹瓷香承', 'THE CRACKLED VESSEL', '298', '手工/限量', 'Crackled.webp'],
  ['闻香颈饰', 'AROMA PENDANT', '368', '含扩香芯', 'Necklace .webp'], 
  ['黑胡桃扩香木', 'WALNUT DIFFUSER WOOD', '198', '北美黑胡桃', 'Walnut.webp'],
  ['氛围香氛烛', 'ATMOSPHERE CANDLE', '228', '200g', 'candle.webp'],
  ['紫晶典藏瓶', 'VIOLET GLASS APOTHECARY', '158', 'Miron紫晶', 'Vessel.webp']
];
const jingDataM = [
  ['天然草本线香', 'NATURAL HERBAL INCENSE', '128', '30支/管', 'Incense Sticks.webp'],
  ['随行滚珠瓶', 'TRAVEL ROLLER BOTTLE', '88', '10ml', 'Rollerball.webp'],
  ['旷野扩香石', 'RAW DIFFUSER STONE', '168', '天然原石感', 'Gypsum.webp'],
  ['归真座', 'THE MOUNTAIN BASE', '258', '陶/石复合', 'mountain.webp'],
  ['承露玻璃罩', 'HANDBLOWN GLASS CLOCHE', '328', '手工吹制', 'glass.webp']
];
...
Object.entries(PROVINCE_GROUPS).forEach(([sub, list]) => {
  list.forEach((prov) => {
    addD(`cn_${prov}`, prov, prov.toUpperCase(), '亚洲', 5, REGION_VISUALS.china, getProducts(prov), 'arrived', true, sub);
  });
});
