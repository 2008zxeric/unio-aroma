...
// 1. 元系列 (Origin)
const YUAN_DEFS = [
  { g: '金 · 肃降', slug: 'gold', items: ['神圣乳香', '极境薄荷', '极境尤加利', '极境茶树', '极境香茅'] },
  { g: '木 · 生发', slug: 'wood', items: ['老山檀香', '极境丝柏', '极境雪松', '极境松针', '神圣花梨木'] },
  { g: '水 · 润泽', slug: 'water', items: ['极境杜松', '极境岩兰草', '极境广藿香', '极境没药', '极境安息香'] },
  { g: '火 · 释放', slug: 'fire', items: ['大马士革玫瑰', '极境依兰', '极境茉莉', '极境橙花', '极境天竺葵'] },
  { g: '土 · 稳定', slug: 'earth', items: ['极境佛手柑', '横断生姜', '极境红橘', '极境葡萄柚', '极境橡木苔'] }
];
...
YUAN_DEFS.forEach((group) => {
  group.items.forEach((name) => {
    const itemSlug = EN_MAP[name] || 'essence';
    const id = `yuan_${group.slug}_${itemSlug}`;
    DATABASE[id] = {
...
const JING_DEFS = [
  { g: '场 · 净愈', items: [
    ...
    { n: '归藏 · 铝合金旅行扩香盒', e: 'Alloy Travel Scent Box', ing: '航空铝材 / 精密切削', img: ASSET_REGISTRY.visual_anchors.space }
  ]},
  { g: '意 · 冥想', items: [
...
JING_DEFS.forEach((group, gIdx) => {
  group.items.forEach((item, i) => {
    const id = `jing_${gIdx === 0 ? 'field' : 'meditation'}_${i + 1}`;
    DATABASE[id] = {
...
const ERIC_REAL_MAP = {
  '亚洲': [
    { n: '泰国', c: 40, pIds: ['yuan_fire_rose', 'he_body_2'] },
    { n: '中国', c: 23, pIds: ['yuan_gold_essence', 'he_body_1'], isHome: true },
    { n: '中国香港', c: 18, pIds: ['he_mind_5'] }
  ],
  '欧洲': [
    { n: '土耳其', c: 8, pIds: ['yuan_fire_rose'] }
  ]
};
...
Object.entries(CHINA_MAP).forEach(([sub, list]) => {
  list.forEach((name, i) => {
    ...
    let pIds: string[] = [];
    if (name === '西藏') pIds = ['yuan_gold_essence']; 
    if (name === '云南') pIds = ['yuan_fire_rose']; 
